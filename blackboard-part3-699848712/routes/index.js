...
router.get('/messages-page', async function(req, res, next) {
  ...
  // ====> Récupère les messages de l'admin : 
...
/* GET chart page - Affichage des graphiques */
router.get('/charts', async function(req, res, next) {
  ...
  //  Chiffre d’affaires par mois d’une année :

  var aggr = orderModel.aggregate()  // On initialise l'aggregation sur orderModel

  aggr.match({status_payment:"validated"})  // Récupère que les paiements qui sont validés

  // Regroupe les informations.
  // l'id est une combinaison. C'est un objet qui contient l'année et le mois.
  // CA somme du total de chaque commande
  aggr.group({ _id: {year: {$year:'$date_insert'}, month:{$month: '$date_insert'}}, CA:{$sum: '$total'}})
  
  // trie le résultat en lui précisant la propriété à trier
  // 1: trie de manière ascendante
  aggr.sort({ _id : 1})

  var totalCAByMonth = await aggr.exec()

  // Comment fournir un tableau à notre fichier ejs
  // Il faut utiliser JSON.stringify côté front
